...
This notebook, `preprocessing.ipynb`, is designed for the initial data loading, exploration, and basic preprocessing steps for a housing price prediction task. It covers the following key areas:
...
```
# model-training notebook : Satellite + Tabular Property Price Prediction
(Kaggle Notebook)

This notebook builds and compares three approaches for property price prediction:
...
- `model_comparison.png` — RMSE / R² / MAPE comparison
- `gradcam_visualization.png` — Grad-CAM heatmaps
- `sample_images.png`, `prediction_samples.png` — diagnostic visuals
...
- `data/train.xlsx` — training table
- `data/test.xlsx` — test table
- `mapbox_images/` — directory with satellite images named `img_0.png` ... `img_N.png`
...
- `PropertyDataset` aligns images using the original DataFrame row indices — ensure image availability matches row count.
